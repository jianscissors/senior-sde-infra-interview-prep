...
**Current role:** SRE on the Cloud Network team at Alibaba Cloud, focused on the load balancer (LB) product and its controller — on-call across LB/controller components. Targeting a move back to SDE; see the "Current-role / career-narrative questions" section in `../interview-questions/03-behavioral-and-leadership-questions.md` for how to frame this transition. Primary evidence story is `#cloud-network-automation` in `star-stories.md` (built an automated diagnosis tool for unclassified LB upgrade/change errors — cut weekly investigation time from ~2 days to ~5 minutes and raised the SDE offload rate from 50% to 90%) — lead with this whenever the "why SDE" question comes up.
